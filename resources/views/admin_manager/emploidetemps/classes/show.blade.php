        ...
        <div class="dashboard-page-one">
            ...
            <div class="dashboard-content-one">
                ...
                <div class="breadcrumbs-area">
                    <h3>@lang('Class Schedule')   -->  {{$classe->nom}}</h3>
                    <ul>
                        <li>
                        ...
                        <tbody>
                            @foreach($periodes as $periode)
                                ...
                                @foreach($jours as $jour)    
                                    @php
                                        //
                                        $classe_matiere_id_teacher = DB::table('classe_matiere')->select('id')->where('classe_id', $classe->id)->pluck('id')->toArray();
                                        ...
                                        @if(in_array($jp_id->id, $jour_periode_id_teacher))
                                            <td rowspan="" class="align-middle text-center" style="background-color:AliceBlue;" > 
                                                @php
                                                    ...
                                                    $classe_matiere = DB::table('classe_matiere')->select('classe_id','matiere_id','user_id')->whereIn('id',$classe_matiere_id)->where('classe_id',$classe->id)->first();
                                                    if($classe_matiere->user_id != null){
                                                @endphp
                                                        <span style="color:black;" >{{App\Models\Matiere::findOrFail($classe_matiere->matiere_id)->nom}}<br/>Teacher : {{App\Models\User::findOrFail($classe_matiere->user_id)->name}}</span>
                                                    @php
                                                    }else{
                                                    @endphp
                                                        <span style="color:black;" > {{App\Models\Matiere::findOrFail($classe_matiere->matiere_id)->nom}}<br/>No teacher </span>                                                        
                                                    @php
                                                    }
                                                    @endphp
                                            </td>
