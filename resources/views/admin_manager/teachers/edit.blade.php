            ...
            <div class="dashboard-content-one">
                ...
                <div class="card height-auto">
                    <div class="card-body">
                        ...
                        <form class="new-added-form" method="POST" action="{{ route('dashboard_manage.teachers.update', $teacher->id)}}" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            ...
                            <div class="row">
                                <div class="col-xl-4 col-lg-6 col-12 form-group">
                                    ...
                                    <input type="text" value="{{$teacher->name}}" name="name" placeholder="" class="form-control">
                                    @error('name')
                                    <span class="form-text text-muted" role="alert"><strong class="text-danger">{{ $message }}</strong></span>
                                    @enderror
                                </div>
                                ...
                                <div class="col-xl-4 col-lg-6 col-12 form-group">
                                    <label>E-Mail *</label>
                                    <input type="email" value="{{$teacher->email}}" name="email" placeholder="" class="form-control">
                                    @error('email')
                                        <span class="form-text text-muted" role="alert"><strong class="text-danger">{{ $message }}</strong></span>
                                    @enderror
                                </div>  
                                ...
                                <span class="form-text text-muted" role="alert"><strong class="text-danger avatar" ></strong></span>
                                <div class="col-xl-4 col-lg-6 col-12 form-group">
                                    <label>@lang('Matieres') *</label>
                                    <select name="matiere[]" class="select2" multiple required>
                                        <option disabled value="multiselect">@lang('Please Select Subject') *</option>
                                    @foreach($classe_matieres as $value)
                                        <option @if( $value->user_id === $teacher->id ) selected @endif value="{{ $value->matiere_id . ',' . $value->classe_id }}">
                                            {{ App\Models\Matiere::findOrFail($value->matiere_id)->nom }}
                                            ({{ App\Models\Classe::findOrFail($value->classe_id)->nom }})
                                        </option>
                                    @endforeach
                                    </select>
                                    @error('matiere')
